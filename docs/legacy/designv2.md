> **Legacy (moved 2026-08-25). Tone section still the best statement of the game; the structure around it is dated.** Wrong or changed since: the working title was never confirmed anywhere else; "5-7 hours" is not the demo target (the demo is Act 1, 45-90 min, last replay ~46 min); Run 0 / the forced bust and the free Poster are cut; the "cheerful institutional" captor voice became the friendly-host House (`docs/house-script.md`); the shove-rate refactor described here was replaced again (see `models/shove_rate.lua`).
