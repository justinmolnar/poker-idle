> **Legacy (moved 2026-08-25). Says "NOT built"; it is built.** Anti-chips (at every stake, inverse ladder, Ultra pays 0), catalog corruption with its UI, the Ultra stake at $10T whose stack loss IS the underflow, the -999% broken grind, and the whole-deck ending into credits all shipped in August 2026. Every number in this doc is a placeholder; the live values are in `data/`. Where the build diverged from the spec: the anti ladder is inverse (T1 highest); R3's multiplier is covered by a card like the Act 1 cheat, not zeroed forever; corruption is per item with money + loss families, no 100% chances.
