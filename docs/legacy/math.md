> **Legacy (moved 2026-08-25). The core formula here is dead.** It says R2 = catalog x mult/2 and R3 = catalog/2 x mult/2. The live model (`models/shove_rate.lua`) is R1 = (items + deck) x mult, R2 = deck x mult (card 6 covers the items), R3 = deck x mult3 (card 7 covers the bank; 0 until the underflow, then 999). Anti-chips, the deck cap at things-you-own, the master capstone and the underflow are not in this doc. Read the code, not this.
