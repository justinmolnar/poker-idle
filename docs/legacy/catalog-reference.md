> **Legacy (moved 2026-08-25). Snapshot of 2026-08-05, wrong since.** 48 items then; 52 now. 16 renamed in August (Whiteboard -> Corkboard, Copy Machine -> Receipt Printer, Wooden Door -> Reserved Seat Card, Frosted Glass Door -> High Roller Pass, Claw-Foot Tub -> Blackout Curtains, Utility Sink -> Electric Kettle, etc.) and four Computer Accessories added. Per-item shove is flat 1% (`Balance.K_SHOVE_PER_ITEM`), so the per-line shove numbers here mean nothing. Regenerate from `data/catalog.lua` if a reference is wanted.
