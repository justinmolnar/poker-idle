> **Legacy (moved 2026-08-25). Status never updated.** Refactor A (juice: sounds, flights, pops, shaders, chip piles) largely shipped through the summer. Refactor B (poker theater: blinds, betting rounds, folds) exists in some form in `models/HandScript.lua`. The doc still reads as a proposal for both.
