...
3. Shove rate climbs visibly during the grind — bankroll multiplies your catalog-derived shove rate, hit PP bounties for small additive bumps. Every dollar earned makes the eventual all-in stronger.
...
- Shove rate is a product, not a sum (see [Shove Rate System](#shove-rate-system) below). Both catalog progression and current-run bankroll must be developed; neither alone is sufficient.
...
1. **Runout 1:** Standard NLHE all-in. Hole cards, flop, turn, river. Shove rate determines result.
2. **Runout 2** (if runout 1 wins): The dealer adds an extra community card. The hand reruns with the new card included.
3. **Runout 3** (if runout 2 wins): Another extra card. The hand reruns again.
...
The gauntlet is itself a runner-runner-runner — three improbable consecutive wins. The whole game is tournaments all the way down.
...
The cheat is silently revealed, not pre-explained. The first time the player wins runout 1 — which doesn't happen until they've built meaningful shove rate — the dealer slides another card onto the felt and the hand resolves against them. That is the demo's climax (see [Demo Structure](#demo-structure)).
...
- The escalating visual is the cards. By runout 3, eight community cards on the felt. The accumulation is the climax.
...
pp_bonus      = pp_banked_this_run × 0.005  (small additive sprinkle)

total_rate = (base_rate × bankroll_mult) + pp_bonus
...
- **PP banked** = small in-run sprinkle. Rewards hitting bounties this attempt.

Three distinct rhythms, no overlap. Each source has a clear role.
...
Payouts (multiples of buy-in): **3× / 6× / 20×** at hands 6 / 7 / 8 respectively. Boosted by catalog perks (Plastic Trophy → 4/8/20, Engraved Plaque → 5/10/20).
...
- Fill outcome-model dimensions (Sharper Reads → `win_chance`, Pot Building → `win_dist`, Pot Control → `loss_dist`, Focus → table capacity, Hire/Cursor Speed → cursor swarm).
...
- Catalog rename pass (poker-ability names → object names).
...
- Shove rate (multiplicative product of catalog × bankroll mult + PP bonus, no hard cap).
...
- **Stakes tiers?** Six (T1 $0.01/$0.02 through T6 $500/$1000), 10× jumps each.
- **Run-upgrade vs. catalog overlap?** Three distinct sources for shove rate: catalog (permanent base), bankroll (run multiplier), PP banked (run sprinkle). No overlap.
...
- **How many catalog items?** Target ~30–50 for full game. VR ships with current ~14 + 2 MTT items.
