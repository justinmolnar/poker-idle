...
-- views/TablePanel.lua reads to render felt color, header chrome, and a
-- chip-color tint multiplier.
--
...
--
-- The PANEL BORDER is deliberately NOT part of this: every table wears the
-- same default border line, because a colored border is the banked-bounty
-- signal (gold = {chip} banked, purple = {achip} banked) and a per-stake
-- border trim counterfeits it. Stake identity lives on the felt.
--
...
return {
    -- T1 — $0.01/$0.02. Scuffed cheap-card-room olive.
    s001 = {
    ...
    -- T3 — $0.50/$1. Blue felt. More upscale.
    s003 = {
    ...
    -- T7-T10: a felt each, so the stake still reads above T6. Header falls
    -- back to the defaults until they get a full theme.
