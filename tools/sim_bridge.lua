...
local function evalOne(req, exclude)
    ...
    local resp = {
        ...
        stack_pct      = p.win_chance * (p.win_dist.jackpot or 0) * 100,
        loss_stack_pct = (1 - p.win_chance) * (p.loss_dist.jackpot or 0) * 100,
...
function Bridge.info(_)
    ...
    return json.encode({
        stakes = stakes, gtypes = gtypes, upgrades = upgrades, catalog = catalog,
...
function Bridge.bounty(jreq)
    ...
    for i, lane in ipairs(req.lanes or {}) do
        ...
        local award = math.floor((stake.chip_award or 0) * mult + 0.5)
                    + (ctx.jackpot_chip_add or 0)
        -- Odds of the jackpot-tier win that banks the bounty, for the
        ...
        local jp = ok and (res.wc * (res.wd.jackpot or 0)) or 0
        out[i] = { stake_id = lane.stake_id, gtype_id = lane.gtype_id,
                   award = award, jp_per_hand = jp,
                   hands_to_jp = (jp > 0) and (1 / jp) or nil }
    end
